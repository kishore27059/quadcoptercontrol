...
def callback2(rcReceiver):
        ...
        sampleTime = 0
        setpoint = 0
        err_pitch = float(pitch)*(180 / 3.141592653) - rcReceiver.channelData[0]
        err_roll = float(roll)*(180 / 3.141592653) - rcReceiver.channelData[1]
        err_yaw = float(yaw)*(180/3.14159263) - rcReceiver.channelData[3]
        currTime = time.time()
        ...
        esc_br = rcReceiver.channelData[2] + output_roll + output_pitch - output_yaw
        #bl in my code is br in gazebo's world
        esc_bl = rcReceiver.channelData[2] + output_roll - output_pitch + output_yaw
        #fl in my code is bl in gazebo's world
        esc_fl = rcReceiver.channelData[2] - output_roll - output_pitch - output_yaw
        #fr in my code is fl in gazebo's world
        esc_fr = rcReceiver.channelData[2] - output_roll + output_pitch + output_yaw
...
def listener():
        ...
        rospy.Subscriber("imu/data_filtered", Imu, callback1)
